...
![Version: 0.16.2](https://img.shields.io/badge/Version-0.15.4-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 2.0.1](https://img.shields.io/badge/AppVersion-2.0.1-informational?style=flat-square)
...
| settings.omc.actor.id | string | `nil` | Recommended to pin this explicitly. Left unset, it defaults to a freshly-generated UUID on every `helm upgrade`, which now triggers a pod restart on every upgrade (not just real config changes) since ConfigMap/Secret content is hashed into the deployment's rollout trigger |
